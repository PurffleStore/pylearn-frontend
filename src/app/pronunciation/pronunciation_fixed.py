    ...
    for model_info in models:
        try:
            ...
            if phoneme_output:
                ...
                # FIX Bug 2: Translate eSpeak notation to UK IPA symbols
                phonemes = translate_espeak_to_uk(raw_phonemes, word)
                text = ''.join([p for p in phonemes if p.isalpha()])
            else:
                # Text models: use WORD_TO_PHONEMES (single source of truth)
                # FIX Bug 3: No longer uses the inconsistent text_to_phonemes() dict
                text = transcription.strip().lower()
                phonemes = WORD_TO_PHONEMES.get(
                    text,
                    [c for c in text if c.isalpha()]  # fallback: characters only
                )
